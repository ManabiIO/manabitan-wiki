...
- Real third-party names restored; TTS issue points to upstream #864; nonexistent MeCab installer and contradictory Forvo recommendations removed.
- Storage troubleshooting no longer assumes IndexedDB only or recommends destructive reset before backups.
- Privacy describes network requests, optional integrations, normal network metadata, diagnostics, alarms, and Chromium offscreen permission. The website and extension are distinguished.
- Support goes to the Manabi Discord while Issues are disabled. The wiki has its own contribution instructions.
- The homepage no longer presents inherited screenshots/videos as current Manabitan captures. No automatic third-party video embed, remote Google Font, JavaScript-only reveal, browser guessing, or timed slideshow/typewriter is needed. Original upstream assets remain in repository history.
...
- Qualify old/new settings imports, full collection restoration including external resources, custom CSS/templates, MeCab registrations, external API integrations, and AnkiDroid with explicit versions. Documentation does not substitute for those tests.
- Supply current screenshots or a recording from a named released build before adding a product demo back. No fabricated or relabeled upstream screenshots are used as evidence.
